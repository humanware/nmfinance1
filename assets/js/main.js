$(document).ready(function () {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        $(".navbar .nav-link").filter(`[href='#${id}']`).addClass("active");
      }
    });
  }, observerOptions);
  ...
  $(document).on("click", "[data-loan-app='true']", function (event) {
    event.preventDefault();
    const newTab = window.open(jotformUrl, "_blank");
    if (!newTab) {
  ...
  if (!prefersReducedMotion) {
    ...
    gsap.to(".cash-advance-card", {
      boxShadow: "0 0 30px rgba(47, 109, 255, 0.35)",
      duration: 2.8,
      repeat: -1,
      yoyo: true,
      ease: "sine.inOut"
